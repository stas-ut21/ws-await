...
class WebSocketAwait extends WebSocket {
     ...
     *
     * @param {(String|url.Url|url.URL)} args.address The URL to which to connect
     * @param {(String|String[])} args.protocols The subprotocols
     * @param {Object} args.options Connection options
     */
    constructor(...args) {
        super(...args);
     ...
     * @param {Boolean} opt.leaveAwaitId The leaving identification parameter in data after receiving with sendAwait
     * @param {Function} opt.packMessage The custom message packaging function before sending
     * @param {Function} opt.unpackMessage The custom message un'packaging function after receiving
     * @param {Function} opt.generateAwaitId The custom identification parameter generation function for sendAwait
     * @param {Function} opt.attachAwaitId The custom installation feature identification parameter for sendAwait
     * @param {Function} opt.extractAwaitId The custom un'installation feature identification parameter for sendAwait
    ...
    static validateSettings(opt) {
        ...
        Object.keys(opt).forEach(key => {
            ...
            if (opt[key] === null && ['packMessage', 'unpackMessage'].includes(key)) {
                return;
            }
    ...
    setSocket(socket, head, maxPayload) {
        ...
        this._receiver.on('message', data => {
            ...
            if (awaitId) {
                ...
                resolve(parseData);
                this.emit('messageAwait', parseData);
                return;
            }
